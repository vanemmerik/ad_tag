...
- **`scripts.js`** - DOM glue only (single-tag inspector; the two-tag compare was removed). `reviewTag()` reads the input and renders the results into `#results`: a verdict bar, context chips, an issues panel, a clickable tag-anatomy strip, and a component breakdown grouped by role (the `GROUPS` map) with decoded values, macro annotations, and compound decomposition (`decompose()`: `cust_params` to a key/value sub-table, `ppsj` to base64 JSON, comma/pipe lists to chips). Tooltips are retained via `getTooltipContent`. Also manages share URLs and `localStorage`. Contains no parsing/requirement logic of its own.
